...
def clipGeoTIFF(inputtif, cliptif, **kwargs): 
    resolution = kwargs.get('resolution', None)
    overwrite = kwargs.get('overwrite', False)

    # LOAD the NDVI (input) file to get its geotransform and projection
    input_gobj = gdal.Open(inputtif)
    if input_gobj is None:
        raise IOError('Provided input file path is not valid!')
    
    # Get the geotransform and projection of the input file (NDVI)
    input_geotransform = input_gobj.GetGeoTransform()
    input_projection = input_gobj.GetProjection()
    input_raster = input_gobj.GetRasterBand(1)
    input_data = input_raster.ReadAsArray()
    
    # Get the dimensions of the input raster (NDVI)
    input_cols = input_gobj.RasterXSize
    input_rows = input_gobj.RasterYSize
    
    # Open the clip file
    clip_gobj = gdal.Open(cliptif)
    if clip_gobj is None:
        raise IOError('Provided clip file path is not valid!')
    
    # Get the geotransform and projection of the clip file
    clip_geotransform = clip_gobj.GetGeoTransform()
    clip_projection = clip_gobj.GetProjection()
    clip_raster = clip_gobj.GetRasterBand(1)
    clip_data = clip_raster.ReadAsArray()
    
    # Calculate the overlap between the two rasters based on their geotransforms
    # We assume the input raster defines the area we want to clip from the second raster
    Ix_input, pixel_width, _, Iy_input,_, pixel_height = input_geotransform
    Ix_clip, pixel_width_clip, _, Iy_clip, _, pixel_height_clip= clip_geotransform
    
    # Calculate the coordinates of the clipped area in terms of pixel indices
    x_start = int((Ix_input - Ix_clip) / pixel_width_clip)
    y_start = int((Iy_input - Iy_clip) / pixel_height_clip)
    x_end = int((Ix_input + input_cols * pixel_width - Ix_clip) / pixel_width_clip)
    y_end = int((Iy_input + input_rows * pixel_height - Iy_clip) / pixel_height_clip)
    
    # Clip the second raster (cliptif) using these calculated indices
    clipped_data = clip_data[y_start:y_end, x_start:x_end]

    # Create the new geotransform for the clipped raster
    new_geotransform = (Ix_clip + x_start * pixel_width_clip, pixel_width_clip, 0,
                        Iy_clip + y_start * pixel_height_clip, 0, pixel_height_clip)

    return clipped_data, new_geotransform, clip_projection        
